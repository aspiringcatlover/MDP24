...
public class Exploration {
	...
	public Robot explore() {
		...
		while (actual_percentage < goal_percentage && System.currentTimeMillis() != endTime) {
			...
			if (robot.getXCoord() == 1 && robot.getYCoord() == 5 && start
					|| robot.getXCoord() == 1 && robot.getYCoord() == 1 && start) {
			...
			/*
			 * if (isSimulated){ try { // ms timeout int timeout = (1 / steps_per_sec) *
			 * 1000; Thread.sleep(timeout); // Customize your refresh time } catch
		...
		while (!unexploredGrids.isEmpty()) {
			/*
			 * go to unexplored grid & continue to senseMap find out grid is obstacle or
			 * what then unexploredGrids = getUnexploredGrid();
			 */
			...
			// System.out.println("unexplored grid:" + nearestUnexploredGrid.getHorCoord()+"
			// " +nearestUnexploredGrid.getVerCoord());
			pathFinder = new PathFinder(map); // do singleton or smth
			System.out.println("what happen start:" + robot.getXCoord() + " " + robot.getYCoord() + " end:"
					+ nearestUnexploredGrid.getHorCoord() + " " + nearestUnexploredGrid.getVerCoord());
			...
			while (path == null) {
				...
				for (int i = 1; i <= 2; i++) {
					if (unexploredGridX == 0 || unexploredGridY == 0 || unexploredGridX == 14
							|| unexploredGridY == 19) {
						...
						for (int y = -2; y < 3; y++) {
							for (int x = -2; x < 3; x++) {
								...
								System.out.println("robot current position: " + robot.getXCoord() + " "
										+ robot.getYCoord() + " grid to go to: " + (unexploredGridX + x) + " "
										+ (unexploredGridY + y));
								System.out.println("check surrounding grid: "
										+ checkSurroundingGrid(unexploredGridX + x, unexploredGridY + y));
								System.out.println(
										"check robot coordinate x" + (robot.getXCoord() != unexploredGridX + x));
								System.out.println(
										"check robot coordinate x" + (robot.getYCoord() != unexploredGridY + y));
								// &&
								if ((robot.getXCoord() != unexploredGridX + x
										|| robot.getYCoord() != unexploredGridY + y)
										&& checkSurroundingGrid(unexploredGridX + x, unexploredGridY + y)) {
									System.out.println("robot current position: " + robot.getXCoord() + " "
											+ robot.getYCoord() + " grid to go to: " + (unexploredGridX + x) + " "
											+ (unexploredGridY + y));
									path = pathFinder.getShortestPath(robot.getXCoord(), robot.getYCoord(),
											unexploredGridX + x, unexploredGridY + y);
								}
						...
						for (int y = -1; y < 2; y++) {
							for (int x = -1; x < 2; x++) {
								...
								System.out.println(
										"check robot coordinate x" + (robot.getXCoord() != unexploredGridX + x));
								System.out.println(
										"check robot coordinate y" + (robot.getYCoord() != unexploredGridY + y));
								if ((robot.getXCoord() != unexploredGridX + x
										|| robot.getYCoord() != unexploredGridY + y)
										&& checkSurroundingGrid(unexploredGridX + x, unexploredGridY + y)) {
									System.out.println("robot current position: " + robot.getXCoord() + " "
				...
				pathFinder = new PathFinder(map); // do singleton or smth
				System.out.println("get next unexplored grid in list: " + unexploredGrids.peek().getHorCoord() + " "
						+ unexploredGrids.peek().getVerCoord());
				nearestUnexploredGrid = unexploredGrids.poll();
				unexploredGridX = nearestUnexploredGrid.getHorCoord();
				unexploredGridY = nearestUnexploredGrid.getVerCoord();
				System.out.println(
						"robot current position: " + robot.getXCoord() + " " + robot.getYCoord() + " grid to go to: "
								+ nearestUnexploredGrid.getHorCoord() + " " + nearestUnexploredGrid.getVerCoord());
				path = pathFinder.getShortestPath(robot.getXCoord(), robot.getYCoord(),
						nearestUnexploredGrid.getHorCoord(), nearestUnexploredGrid.getVerCoord());
				if (path == null) {
			...
			// System.out.println("************");
			if (path != null) {
				moveInstructions = pathFinder.getRobotInstructions(path, robot.getDirection(), robot.getXCoord(),
						robot.getYCoord());
				...
				for (Movement movement : moveInstructions) {
					if (movement.equals(Movement.MOVE_FORWARD)) {
						...
						// System.out.println("^^^^move forward, robot coordinate: " + robot.getXCoord()
						// +" "+ robot.getYCoord());
						robot.moveForward();
						senseMap();
					} else if (movement.equals(Movement.TURN_RIGHT)) {
						// System.out.println("^^^^turn right, robot coordinate: " + robot.getXCoord()
						// +" "+ robot.getYCoord());
						robot.turn(robot.robotRightDir());
						senseMap();

					} else {
						// System.out.println("^^^^move left, robot coordinate: " + robot.getXCoord() +"
						// "+ robot.getYCoord());
					...
					// System.out.println("robot where u at: "+robot.getXCoord()+ " "+
					// robot.getYCoord());

					if (map.getGridCell(unexploredGridY, unexploredGridX).getExplored()) // constantly seeing if the
																							// unexplored grid is
																							// explored alr in case is a
																							// obstacle then the last
																							// move cmi
						break;
					if ((robot.getYCoord() + 1 == unexploredGridY || robot.getYCoord() - 1 == unexploredGridY)
							&& (robot.getXCoord() + 1 == unexploredGridX || robot.getXCoord() - 1 == unexploredGridX)) // if
																														// any
																														// part
																														// of
																														// the
		...
		// if robot not at start point get shortest path to start point & move to start
		// point
	...
	public void rightWallHugging() {
		...
		if (!hasObstacle(robot.robotRightDir())) {
			...
			// simulatorMap.getMap().displayRobotSpace(robot.getXCoord(),
			// robot.getYCoord());
	...
	public void senseMap() {
		...
		case NORTH:
			...
			for (int loc = 0; loc < 3; loc++) {
				...
				for (int i = 0; i < sensor.getGridDistance(); i++) {
					System.out.println(
							"sensor corrdinate: " + x + ", " + (y + i) + "sensor result " + sensorResult.get(i));
					if (sensorResult.get(i) == null)
						break;
					map.setObstacleForGridCell(sensor.getYCoord() + i, sensor.getXCoord(), sensorResult.get(i));
					map.setExploredForGridCell(sensor.getYCoord() + i, sensor.getXCoord(), true);
	...
	public PriorityQueue<GridCell> getUnexploredGrid() {
		PriorityQueue<GridCell> unexploredGrid = new PriorityQueue<>(11,
				(o1, o2) -> Integer.compare(o1.getgCost(), o2.getgCost()));
		 ...
		 * System.out.println("peek pq"+unexploredGrid.peek().getHorCoord() + " "
		 * +unexploredGrid.peek().getVerCoord());
		 * System.out.println("----------------------------------get unexplored grid");
		 * for (GridCell gridCell1:unexploredGrid){ System.out.println("NOT EXPLORED" +
		 * gridCell1.getHorCoord() +" "+gridCell1.getVerCoord()); }
		 */
	...
	private boolean checkSurroundingGrid(int x, int y) {
		...
		return (!map.getGridCell(y + 1, x + 1).getObstacle() && !map.getGridCell(y - 1, x - 1).getObstacle()
				&& !map.getGridCell(y - 1, x + 1).getObstacle() && !map.getGridCell(y + 1, x - 1).getObstacle()
				&& !map.getGridCell(y, x + 1).getObstacle() && !map.getGridCell(y + 1, x).getObstacle()
				&& !map.getGridCell(y - 1, x).getObstacle() && !map.getGridCell(y, x - 1).getObstacle());
	}
